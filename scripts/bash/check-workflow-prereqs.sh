...
if $PATHS_ONLY; then
    if $JSON_MODE; then
        printf '{"UNIT_REPO_ROOT":"%s","UNIT_BRANCH":"%s","UNIT_HAS_GIT":%s,"UNIT_DIR":"%s","UNIT_BRIEF_FILE":"%s","UNIT_BRIEF_JSON_FILE":"%s","UNIT_DESIGN_FILE":"%s","UNIT_DESIGN_JSON_FILE":"%s","UNIT_SEQUENCE_FILE":"%s","UNIT_SEQUENCE_JSON_FILE":"%s","UNIT_AUDIT_REPORT_FILE":"%s","UNIT_AUDIT_REPORT_JSON_FILE":"%s","UNIT_MANIFEST_FILE":"%s","UNIT_CHARTER_FILE":"%s"}\n' \
            "$REPO_ROOT" "$CURRENT_BRANCH" "$HAS_GIT" "$UNIT_DIR" "$BRIEF_FILE" "$BRIEF_JSON_FILE" "$DESIGN_FILE" "$DESIGN_JSON_FILE" "$SEQUENCE_FILE" "$SEQUENCE_JSON_FILE" "$AUDIT_REPORT_FILE" "$AUDIT_REPORT_JSON_FILE" "$MANIFEST_FILE" "$CHARTER_FILE"
    else
