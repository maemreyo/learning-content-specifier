...
function Generate-Commands {
    ...
    $templates = Get-ChildItem -Path "factory/templates/commands/*.md" -File -ErrorAction SilentlyContinue
...
function Generate-CopilotPrompts {
    ...
    $agentFiles = Get-ChildItem -Path "$AgentsDir/lcs.*.agent.md" -File -ErrorAction SilentlyContinue
...
function Build-Variant {
    ...
    $specDir = Join-Path $baseDir ".lcs"
    New-Item -ItemType Directory -Path $specDir -Force | Out-Null
    
    # Copy memory directory
    if (Test-Path "memory") {
        Copy-Item -Path "memory" -Destination $specDir -Recurse -Force
        Write-Host "Copied memory -> .lcs"
    }
    
    # Only copy the relevant script variant directory
    if (Test-Path "factory/scripts") {
        $scriptsDestDir = Join-Path $specDir "scripts"
        New-Item -ItemType Directory -Path $scriptsDestDir -Force | Out-Null
        
        switch ($Script) {
            'sh' {
                if (Test-Path "factory/scripts/bash") {
                    Copy-Item -Path "factory/scripts/bash" -Destination $scriptsDestDir -Recurse -Force
                    Write-Host "Copied factory/scripts/bash -> .lcs/scripts"
                }
            }
            'ps' {
                if (Test-Path "factory/scripts/powershell") {
                    Copy-Item -Path "factory/scripts/powershell" -Destination $scriptsDestDir -Recurse -Force
                    Write-Host "Copied factory/scripts/powershell -> .lcs/scripts"
                }
            }
        }
        
        # Copy shared python tooling consumed by shell wrappers.
    ...
    if (Test-Path "factory/templates") {
        $templatesDestDir = Join-Path $specDir "templates"
        New-Item -ItemType Directory -Path $templatesDestDir -Force | Out-Null
        
        Get-ChildItem -Path "factory/templates" -Recurse -File | Where-Object {
            $_.FullName -notmatch 'factory[/\\]templates[/\\]commands[/\\]' -and $_.Name -ne 'vscode-settings.json'
        } | ForEach-Object {
            $relativePath = $_.FullName.Substring((Resolve-Path "factory/templates").Path.Length + 1)
    ...
    switch ($Agent) {
        ...
        'copilot' {
            ...
            if (Test-Path "factory/templates/vscode-settings.json") {
                Copy-Item -Path "factory/templates/vscode-settings.json" -Destination (Join-Path $vscodeDir "settings.json")
            }
        }
        'cursor-agent' {
