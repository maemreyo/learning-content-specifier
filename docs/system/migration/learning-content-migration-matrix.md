...
|---|---|---|---|---|
| Command templates | `templates/commands/*.md` | Short/underspecified workflows | High | Rewritten with strict sections and hard-gate contracts |
| Script contracts | `scripts/bash/*`, `scripts/powershell/*` | Branch check, schema drift, bool mismatch risk | High | Added skip branch check, UNIT_* schema, deterministic gate validator |
...
| Packaging | `.github/workflows/scripts/create-release-packages.*` | `.lcs.lcs` rewrite bug; agent dir mismatch | High | Safer rewrite regex; AGENTS.md folder conventions |
| Editor onboarding | `templates/vscode-settings.json`, `.devcontainer/devcontainer.json` | Old command recommendations | Medium | Updated to charter/define/design/sequence/author |
